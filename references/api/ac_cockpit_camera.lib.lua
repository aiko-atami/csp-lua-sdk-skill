...
---@alias ac.CockpitCameraMode
---| `ac.CockpitCameraMode.Base` @Regular mode (or a mode with connected TrackIR which hasn’t moved yet).
---| `ac.CockpitCameraMode.IR` @TrackIR mode (input `neck` position and orientation is already offset).
---| `ac.CockpitCameraMode.VR` @VR mode (better to do the absolute minimum there, only some sort of G-forces reaction).
ac.CockpitCameraMode = {
  Base = 0, ---@type ac.CockpitCameraMode #Regular mode (or a mode with connected TrackIR which hasn’t moved yet).
  IR = 1, ---@type ac.CockpitCameraMode #TrackIR mode (input `neck` position and orientation is already offset).
  VR = 2, ---@type ac.CockpitCameraMode #VR mode (better to do the absolute minimum there, only some sort of G-forces reaction).
}

---Stop Neck FX from moving driver model head to match camera position.
---@param disable boolean? @Default value: `true`.
function ac.disableDriverModelSync(disable) end
