...
---@return boolean
function ac.isSkippingFrame() end
...
---@param name string @Name of a section of a screen entity to adjust (i.e. `'NAVIGATOR_0'`).
---@param opacity number? @New opacity, if 0 a screen entity would stop completely. Default value: 1.
---@param offset number|vec2? @Offset for an output of a given screen entity. Default value: `vec2(0, 0)`.
---@param scale number|vec2? @Scale for an output of a given screen entity. Default value: `vec2(1, 1)`.
function ac.setDynamicTextureShift(name, opacity, offset, scale) end
...
---from weather type.
---@return number
function ac.estimateSatelliteOcclusion() end
...
---@param name string @Name of mesh to check, or a filter (use curly brackets and commas to list several several meshes, for example).
---@param uvFrom number|vec2? @UV region, start (from 0 to 1). Default value: `vec2(0, 0)`.
---@param uvSize number|vec2? @UV region, end (from 0 to 1). Default value: `vec2(1, 1)`.
---@param inCarPos vec3|nil @Point in car coordinates distance to which will be checked. Default value: `nil`.
---@param inCarRadius number? @Threshold distance from point in meters. If mouse intersects mesh at point further than that, it’s ignored. Default value: 0.05.
---@param inCarLocalPos boolean? @If set to true, local mesh coordinates are checked. Note: if mesh is scaled, distance would have to be scaled too. Default value: `false`.
---@return boolean
function ac.isMeshClicked(name, uvFrom, uvSize, inCarPos, inCarRadius, inCarLocalPos) end
...
---@param inCarLocalPos boolean? @If set to true, local mesh coordinates are checked. Note: if mesh is scaled, distance would have to be scaled too. Default value: `false`.
---@return boolean
function ac.isMeshPressed(name, uvFrom, uvSize, inCarPos, inCarRadius, inCarLocalPos) end
...
---@param inCarLocalPos boolean? @If set to true, local mesh coordinates are checked. Note: if mesh is scaled, distance would have to be scaled too. Default value: `false`.
---@return boolean
function ac.isMeshHovered(name, uvFrom, uvSize, inCarPos, inCarRadius, inCarLocalPos) end
...
---@param pos vec3
---@param save boolean|`true`|`false` @If `true`, new values will be saved in “view.ini” in “Documents\AC\cfg\cars” a few seconds after a call, otherwise default camera positioning app will get a “Save needed” label.
function ac.setDriverEyesPosition(pos, save) end

---Use `{Lua:KEY}` in your navigator text to get the value.
---@param key string
---@param value string
function ac.setNavigatorText(key, value) end
...
---Access a certain light source defined in car config. If your car needs lights with complex logic, consider using `[LIGHT_EXTRA_…]`
---and control lights via this function rather than creating a new light sources: lights created via config are more customizable and
---have additional options to help with performance, like disabling self-lighting from interior camera or not affecting the track.
---
---Once accessed, reference remains valid the whole time, so you can access light at the start of your script and keep the reference.
---@param name string @Section name.
---@return ac.StateCarLight?
function ac.accessCarLight(name) end
...
--[[ car_scriptable_display/car_scriptable_display_structs.lua ]]
...
local _ac_StateCarRenderingCamera = nil
...
---@field gamma number
local _ac_StateCarTexturePreprocessing = nil
...
local _ac_StateCarLight = nil

--[[ car_scriptable_display/car_scriptable_display_utils.lua ]]
...
---@param params {mirrorIndex: integer, pos: vec2, size: vec2, color: rgbm, ratio: number, alignX: number, alignY: number}|`{mirrorIndex = 0, pos = vec2(), size = vec2(), color = rgbm.colors.white, ratio = 1, alignX = 0, alignY = 0}` "Table with properties:\n- `mirrorIndex` (`integer`): 0-based mirror index (for Real Mirrors).\n- `pos` (`vec2`): Coordinates of the top left corner in pixels.\n- `size` (`vec2`): Size in pixels.\n- `color` (`rgbm`): Mirror will be multiplied by this color.\n- `ratio` (`number`): Optional ratio adjustment multiplier.\n- `alignX` (`number`): Value from -1 to 1 shifting mirror texture horizontally (applicable especially if Real Mirrors are disabled).\n- `alignY` (`number`): Value from -1 to 1 shifting mirror texture vertically."
---@return display.DrawnMirror
function display.mirror(params) end
