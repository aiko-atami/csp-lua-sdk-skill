...
---@param message string
---@param icon string? @Refers to one of 64×64 icons available in CSP. Check [textures\gui\icons_src\icons_64](https://github.com/ac-custom-shaders-patch/acc-extension-config/tree/master/textures/gui/icons_src/icons_64) for available icons. Default value: 'DRIVE'.
---@return boolean
function ac.setStartMessage(message, icon) end

---Specifies which car is being controlled by the user. Uses the same mechanism as walking-out module. Available to New Modes only. Car that loses
---control keeps its input state.
---@param carIndex integer @0-based index of a car, or -1 if user shouldn’t control any car.
function physics.setUserControlledCar(carIndex) end

---@param carIndex integer @0-based index of a car.
---@param disable boolean? @Default value: `true`.
function ac.disableCar(carIndex, disable) end

---Turn a certain car into a ghost of another car. Hacky and very experimental, but let’s you have a ghost with full effects and collisions.
---If target car type is different from source car, visuals will be messed up. CSP replay extensions currently won’t work.
---@param targetCarIndex integer @0-based index of a car that will become a ghost.
---@param sourceCarIndex integer? @0-based index of a car replay of which will be used as the source of data for the ghost. Pass -1 to disable the ghost. Default value: -1.
---@param timeOffset number? @Ghost delay in seconds. If zero or negative, ghost will be disabled. Default value: 0.
---@param collisions nil|'disabled'|'stiff'|'pushable'|'knockable' @Set to `nil` or `'disabled'` to disable collisions. With `'pushable'`, ghost will react to collisions and revert to original trajectory in a few seconds. With `'knockable'`, ghost will stop being a ghost after a hit.
---@return boolean @Return `true` if ghost thing is active.
function ac.setReplayBasedGhost(targetCarIndex, sourceCarIndex, timeOffset, collisions) end

---Override current Weather FX conditions, blocking Weather FX controller entirely.
---@param conditions ac.ConditionsSet @Pass `nil` to disable the override.
---@return boolean
function ac.overrideWeatherConditions(conditions) end

---Add a new window to the main menu with settings related to the mode. Shown at the same place as the chat in an online race.
---@param callback fun()
---@param settings fun()?
---@param heightHint number?
---@return ac.Disposable
function ac.addMainMenuSettings(callback, settings, heightHint) end
...
function ac.endSession(message, successfulRun, sessionResults) end
