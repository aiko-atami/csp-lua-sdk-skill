...
---@alias ac.DualSenseHapticParam
---| `ac.DualSenseHapticParam.Bodywork` @Value: 0.
---| `ac.DualSenseHapticParam.Engine` @Value: 1.
---| `ac.DualSenseHapticParam.Gear` @Value: 2.
---| `ac.DualSenseHapticParam.GearGrind` @Value: 3.
---| `ac.DualSenseHapticParam.Limiter` @Value: 4.
---| `ac.DualSenseHapticParam.Skid` @Value: 5.
---| `ac.DualSenseHapticParam.Wheel` @Value: 6.
---| `ac.DualSenseHapticParam.CollisionCar` @Value: 7.
---| `ac.DualSenseHapticParam.CollisionObject` @Value: 8.
---| `ac.DualSenseHapticParam.CollisionTrack` @Value: 9.
---| `ac.DualSenseHapticParam.ScrapeCar` @Value: 10.
---| `ac.DualSenseHapticParam.ScrapeTrack` @Value: 11.
ac.DualSenseHapticParam = {
  Bodywork = 0, ---@type ac.DualSenseHapticParam #Value: 0.
  Engine = 1, ---@type ac.DualSenseHapticParam #Value: 1.
  Gear = 2, ---@type ac.DualSenseHapticParam #Value: 2.
  GearGrind = 3, ---@type ac.DualSenseHapticParam #Value: 3.
  Limiter = 4, ---@type ac.DualSenseHapticParam #Value: 4.
  Skid = 5, ---@type ac.DualSenseHapticParam #Value: 5.
  Wheel = 6, ---@type ac.DualSenseHapticParam #Value: 6.
  CollisionCar = 7, ---@type ac.DualSenseHapticParam #Value: 7.
  CollisionObject = 8, ---@type ac.DualSenseHapticParam #Value: 8.
  CollisionTrack = 9, ---@type ac.DualSenseHapticParam #Value: 9.
  ScrapeCar = 10, ---@type ac.DualSenseHapticParam #Value: 10.
  ScrapeTrack = 11, ---@type ac.DualSenseHapticParam #Value: 11.
}
...
---@return ac.StateJoypadData
function ac.getJoypadState() end

---@param triggerIndex integer @0 for left, 1 for right.
function ac.setDualSenseTriggerNoEffect(triggerIndex) end

---@param triggerIndex integer @0 for left, 1 for right.
---@param startPosition number @Start position of resistance.
---@param force number @Force of resistance.
function ac.setDualSenseTriggerContinuousResitanceEffect(triggerIndex, startPosition, force) end

---@param triggerIndex integer @0 for left, 1 for right.
---@param startPosition number @Start position of resistance.
---@param endPosition number @End position of resistance (>= start).
function ac.setDualSenseTriggerSectionResitanceEffect(triggerIndex, startPosition, endPosition) end

---@param triggerIndex integer @0 for left, 1 for right.
---@param startPosition number @Position at which the effect starts.
---@param beginForce number @Force applied when trigger <= 0.5.
---@param middleForce number @Force applied when trigger >= 0.5.
---@param endForce number @Force applied when trigger is beyond 1.
---@param frequency number @Vibration frequency of the trigger (more like a 0�1 value to scale between two fixed frequencies).
---@param keepEffect boolean|`true`|`false` @Set to `true` to keep the effect playing when trigger goes beyond 1.
function ac.setDualSenseTriggerExtendedEffect(triggerIndex, startPosition, beginForce, middleForce, endForce, frequency, keepEffect) end

---Overrides haptic vibration parameter for the given key.
---@param gamepadIndex integer
---@param key ac.DualSenseHapticParam
---@param value vec4
function ac.setDualSenseHapticParams(gamepadIndex, key, value) end

---@param angle number
function ac.setCustomDriverModelSteerAngle(angle) end
...
---@class ac.StateJoypadData : ClassBase
---@field gamepadIndex integer
---@field gamepadType ac.GamepadType
---@field steeringSpeed number
---@field steeringGamma number
---@field steeringFilter number
---@field steeringDeadzone number
---@field rumbleEffects number
---@field speedSensitivity number
---@field steerLock number
---@field speedKmh number
---@field gForces vec3
---@field localAngularVelocity vec3
---@field ffb number
---@field mzForce number
---@field steerRatio number
---@field steerLinearRatio number
---@field steerStickX number
---@field steerStickY number
---@field localSpeedX number @Speed of front axle on X axis relative to car.
---@field localVelocity vec3 @Car velocity relative to car.
---@field ndSlipL number @At 1 left tyre is at its grip limit, above 1 for slipping tyre.
---@field ndSlipR number @At 1 left tyre is at its grip limit, above 1 for slipping tyre.
---@field ndSlipRL number @At 1 left tyre is at its grip limit, above 1 for slipping tyre.
---@field ndSlipRR number @At 1 left tyre is at its grip limit, above 1 for slipping tyre.
---@field surfaceVibrationGainLeft number
---@field surfaceVibrationGainRight number
---@field surfaceVibrationLengthLeft number
---@field surfaceVibrationLengthRight number
---@field gas number
---@field brake number
---@field steer number
---@field clutch number
...
---@field lookingLeft boolean
---@field lookingRight boolean
---@field lookingBack boolean
---@field headlightsSwitch boolean
---@field headlightsFlash boolean
---@field changingCamera boolean
---@field horn boolean
---@field shifterSupported boolean
---@field requestedGearIndex integer
---@field mguhMode integer
---@field vibrationLeft number
---@field vibrationRight number
