...
---Sets number of MIP maps for input screen texture. Can be called only during script initialization. Overrides value in manifest for subsequent runs.
---@param mips integer
function ac.setMIPs(mips) end

---Force CSP to draw virtual mirror late, after the main scene. Stops accurate virtual mirror post-processing.
---@param active boolean|`true`|`false`
function ac.postponeVirtualMirror(active) end

---Sets number of MIP maps for input screen texture. Can be called only during script initialization. Overrides value in manifest for subsequent runs.
---Causes script to reload.
---@param density number
function ac.setPixelDensity(density) end

---Turns PP-filter on or off.
---@param active boolean|`true`|`false`
function ac.setFilterActive(active) end

---Returns `true` if triple mode is currently active (enabled and currently applicable, as in, camera is not in a replay mode, for example).
---@return boolean
function ac.isTripleModeActive() end

---Returns relative viewport of a triple screen frame.
---@param index integer
---@return vec4
function ac.getTripleScreenViewport(index) end

---Changes values of a triple screen side, also updating triple state. Does not affect triple screen settings, changes won’t be saved and
---will be reset if something like `ac.setTripleWidth()` is called, great for adjusting those sides live (pretty much no overhead here).
---@param index integer @0-based index from 0 to 2.
---@param left number|vec2
---@param right number|vec2
---@param height number
function ac.setTripleScreenSideValues(index, left, right, height) end
